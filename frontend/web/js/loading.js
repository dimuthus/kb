...
function Loading(Checker, isDiv, callback) {
    var rootContext = document.body.getAttribute("data-root");
    var LoadingImage = rootContext+"/images/loading.gif"; //29x29
    ...
    if (typeof (Checker) == "boolean") {
        ...
        if (Checker) {
            if (document.getElementById("LoadingDivLayer") == null) {
                ...
                $("#LoadingDivLayer").fadeIn(50, function () {
                    if (callback != null) {
                        callback();
                    }
                });
        ...
        if(isDiv) { 
            var parentLayer = $('#'+String(Checker));
        }
        else {
            var parentLayer = $('#'+String(Checker)+' table > tbody').first();
        }           
        ...
        if (document.getElementById("LoadingDivLayer_" + String(Checker)) == null) {
            ...
            $("#LoadingDivLayer_" + String(Checker)).fadeIn(500, function () {
                if (callback != null) {
                    callback();
                }
            });
