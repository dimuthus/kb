...
;(function ( $, window, document, undefined ) {
    ...
    Plugin.prototype._create = function () {
        ...
        this.element.append(
            ...
            <h4>Password must contain following:</h4> \<ul> \
            <li data-criterion="length" class="invalid">Minimum length is 8 characters</li> \
            <li data-criterion="letter" class="valid">No spaces</li> \
            </ul> \
            <h4>Password must contain characters from three of the following four categories:</h4> \
            <ul> \
            <li data-criterion="capital" class="invalid">(a) English uppercase characters [A-Z]</strong></li> \
            <li data-criterion="smaller" class="invalid">(b) English lowercase characters [a-z]</li> \
            <li data-criterion="number" class="invalid">(c) Base 10 digits [0-9]</li> \
            <li data-criterion="symbol" class="invalid">(d) Non-alphabetic characters \[#$%&()*+!@<=>_!]\</li> \
            </ul> \
            </div>');
    ...
    Plugin.prototype._bind_input_events = function($s) {
        ...
        $s.bind('keyup', function () {
            ...
            var symbol=password.match(symbols) ? 1 : 0;
            var containWhiteSpace = password.indexOf(' ') >= 0 ? 1 : 0;

            var total = characters + capitalletters + loweletters + number + symbol;
...
//alert(total);
            ...
            that._update_info('smaller', loweletters);
            that._update_info('number', number);
            that._update_info('symbol', symbol);
